...
export function createAdminMethods(client: ApiClient) {
  return {
     ...
     *
     * Aggregates: total views, purchases, saves, average rating, review count,
     * and total revenue generated by the product.
     *
     * @param productId - Product ID to fetch stats for.
     * @returns Analytics snapshot for the given product.
     */
    async getProductAnalytics(productId: number): Promise<ProductAnalytics> {
      try {
        const response = await client.get<{ success: boolean; stats: ProductAnalytics; error?: string }>(
          config.endpoints.analytics.stats(productId)
        );
