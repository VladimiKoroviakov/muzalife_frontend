 ...
 * {@link ApiService} class exported as a singleton (`apiService`).
 *
 ...
 * - The {@link CacheManager} is used to cache hot data (products, FAQs,
 *   polls) so the UI remains responsive between navigations.
 *
 * **Interaction with auth:** after a successful login / OAuth callback the
 * returned JWT is written to `localStorage` via {@link ApiService.setToken}.
 * On logout {@link ApiService.clearUserData} purges the token and all user
 * cache entries.
 *
 * @module services/api
 */
